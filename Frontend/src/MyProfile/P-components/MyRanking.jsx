...
function MyRanking({ stats }) {
  ...
  const currentXP = stats.totalStars * 10;
  let nextTierXP = 1000;
  let nextRank = "Grandmaster";
  
  if (currentXP < 100) { nextTierXP = 100; nextRank = "Advanced Debugger"; }
  else if (currentXP < 400) { nextTierXP = 400; nextRank = "Master Debugger"; }
  else if (currentXP < 1000) { nextTierXP = 1000; nextRank = "Grandmaster"; }

  const progressPercentage = Math.min(100, (currentXP / nextTierXP) * 100);
    ...
    <div className="  rounded-2xl   p-6 font-sans mx-auto shadow-2xl">
      ...
      <div className="flex flex-col items-center mb-10">
        ...
        <p className="text-[#4b6382] font-mono text-[13px] mt-4">
          out of <span className="text-[#5a769a]">{stats.totalMembers}</span> members
        </p>
      </div>
      ...
      <div className="mb-10 w-full">
        <div className="flex justify-between items-end mb-2">
          <span className="text-[#7c98b6] font-mono text-[13px]">XP Progress ({stats.rank})</span>
          <span className="text-[#38bdf8] font-mono text-[13px]">
            {currentXP} <span className="text-[#4b6382]">/ {nextTierXP}</span>
          </span>
        </div>
        ...
        <div className="flex text-xs font-mono text-[#4b6382] mt-3">
          <span>{nextTierXP - currentXP} XP to unlock</span>
          <span className="text-[#7c98b6] ml-2 flex items-center gap-1">
            <Zap size={12} className="text-orange-500 fill-orange-500" /> {nextRank}
          </span>
        </div>
      </div>
      ...
      <button className="w-full flex items-center bg-[#131c2e] hover:bg-[#1a263e] transition-colors border border-white/5 rounded-xl p-3 group">
        ...
        <div className="flex flex-col items-start ml-4 flex-1">
          ...
          <span className="text-[#4b6382] font-mono text-xs mt-0.5">
            See all {stats.totalMembers} rankings
          </span>
        </div>
